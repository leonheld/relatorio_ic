...
O sistema original foi criado usando uma plataforma embarcada da \textit{Texas Instruments}\textsuperscript{\textregistered} codenome \texttt{LAUCHPAD-XL} contendo o chip \textit{CC2640R2}, e suportando a tecnologia \textit{BLE 5.0}, \textit{Bluetooth Low Energy 5.0}. A empresa também nos disponibiliza, como parte do pacote onde a plataforma está inserida, diversos \textit{softwares} e \textit{firmwares} que podem ser utilizados para programar nossas próprias aplicações utilizando a tecnologia Bluetooth. O kit de desenvolvimento utilizado foi o \textit{SDK 1.50.00.58}. Os novos SDK também incluem as ferramentas necessárias para realização do projeto, mas nos momentos iniciais a aclimação com as novas ferramentas se mostrou muito trabalhosa para o porte de todo o código que já havia sido escrito.
