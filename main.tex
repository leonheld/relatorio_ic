...
Como pode ser observado, ambos os módulos contam com dois processadores: ARM Cortex-M3 e ARM Cortex-M0. O primeiro atua como processador de propósito geral, supervisionando os pinos de entrada e saída (\textit{I/O}) de cada placa. O Cortex-M0 controla o circuito de radiofrequência (\textit{RF}) responsável pela implementação do protocolo \textit{Bluetooth} da placa. 

Existe um protocolo de comunicação entre os dois processadores (não acessível ao usuário da placa) que serve para transmissão de dados pelo Bluetooth. Fazemos uso, então, do \textit{Cortex-M3} para acessar e obter dados dos sensores ou periféricos. Estes, por sua vez, são enviados via para para o módulo de recepção (segundo kit\texttt{LAUCHPAD-XL}) usando a API da \textit{TI}.
 %TODO: Referência do documento da API da TI

 Algumas limitações de hardware ainda são realidade para essa tecnologia, fato que, por extensão, limita a escolha de placas de desenvolvimento no mercado. A tecnologia \textit{BLE 5.0}, embora o protocolo da presente versão tenha sido homologado em 2016, ainda não é totalmente adotada e nem amplamente utilizada por vários dispositivos. Visto que o ecossistema do projeto original já estava baseado nesta plataforma, o atual projeto continuou sendo desenvolvido na mesma.
...
Por este software também disponibilidade exemplos de aplicação que podem ser extendidos e modificados livremente, sob licença aberta. Um desses é o \textit{Simple Port Profile}, que é um firmware para conectar duas placas de desenvolvimento \textit{LAUNCHPAD-XL} de tal forma a transferir dados numa configuração bidirecional entre as duas.
...
Para a implementação desse \textit{feature}, foi utilizada uma interrupção externa do \textit{client} (\textbf{Módulo 3}), disparada pelo botão esquerdo do mesmo (podendo ser reconfigurado para qualquer pino de propósito geral da placa). Como esse módulo fica perto do paciente, é possível realizar a operação sem movimentação até o notebook ou até o server (\textbf{Módulo 4}).
...
\begin{enumerate}
    ...
    %figura da parte do código de operação
    ...
    %figura do loop no terminal
    \item Esse código de tratamento levanta um evento que envia via \textbf{Módulo 4}, por bluetooth, um outro código de operação que é detectado pelo \textbf{Módulo 3}, inicializando a operação de transmissão e aquisição dos dados.
    %figura da "aquisição inicializada"
\end{enumerate}

Esse método foi particularmente escolhido pois estabelece uma comunicação entre todas as partes do sistema, que pode ser utilizado para interrupções externas com outros fins, não somente a inicializar de aquisição remota dos dados. 

Essa arquitetura de comunicação entre as partes do sistema também pode ser chamada de \textit{Shell}.
